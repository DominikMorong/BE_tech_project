<script lang="ts">
	import { A, Avatar, Card, Modal } from 'flowbite-svelte';
	import { API } from '../../../api';
...
<Card img={speaker.imgUrl}>
	<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
		{speaker.firstName}
		{speaker.lastName}
	</h5>
	<p class="font-normal text-gray-700 dark:text-gray-400 leading-tight">
		{speaker.descriptionShort}
	</p>
	<A class="font-medium hover:underline" on:click={() => (clickOutsideModal = true)}>Read more</A>
</Card>
...
>
	<div class="flex">
		<Avatar src={`${API.ENDPOINTS.BASE_IMG}${speaker.imgUrl}`} size="xl" rounded class="mr-4" />
		<p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
			{speaker.description}
		</p>
	</div>
	<div class="flex !mt-4">
		<a href={speaker.xFormerlyKnownAsTwitterProfile}
		...
		>
		<a href={speaker.facebookProfile}
			><div class="h-4 w-4 !mt-2 mr-2">
